...
import { Currency, Fullscreen } from "lucide-react";
import { useState } from "react";




export function EnterEmail (props:{setCurrentIndex: (index:number) => void, setEmail: (email: string) => void, currentIndex:number}){
...
const emailFunction = async (event:React.SyntheticEvent<HTMLFormElement>) => {
    ...
    return(
        <div>
          <div> <h3 className="text-[24px] font-semibold">
          Create your account   
        </h3>
        <p className="text-#71717A font-4 font-normal">
        Sign up to explore your favorite dishes.
        </p></div>
            <form method="post" onSubmit={emailFunction} className="flex flex-col gap-4">
