...
export default function CreateUserPage() {
  ...
  const signUp = async (password: string, email:string) => {
      ...
      try {
        const data  = await axios.post("https://food-delivery-backend-roan-three.vercel.app/user/signUp", {
          email: email,
          password: password,
        });
  ...
  return (
    <div className="h-screen flex items-center gap-5 p-4">
      <div className="flex flex-col w-[40%] pl-25 gap-2 h-fit ">
        ...
        {currentIndex == 1 && (
          <CreatePassword signUp={signUp} email={email}  setCurrentIndex={setCurrentIndex}/>
        )}
      </div>
